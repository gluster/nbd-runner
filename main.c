...
nbd_parse_volfile(const char *cfg, char **volume, char **host, char **file)
{
    ...
    sep = strchr(tmp, '@');
    if (!sep) {
        nbd_err("argument '<volname@host:/path>' (%s) is incorrect\n",
                cfg);
        ret = -1;
        goto out;
    }
...
nbd_create_file(int count, char **options)
{
    ...
    if (count != 5 && count != 7) {
        nbd_err("Invalid arguments for create command!\n");
        return -1;
    }
    ...
    if (size < 0) {
        nbd_err("Please specify the file size!\n");
        ret = -1;
        goto out;
    ...
    if (!glfs_access(glfs, file, F_OK)) {
        nbd_err("file with name %s already exist in the volume %s\n",
                file, volume);
        ret = -1;
        goto out;
    }
    ...
    if (!fd) {
        nbd_err("glfs_creat() on volume %s for file %s failed, %s\n",
    ...
    if (ret) {
        nbd_err("glfs_ftruncate() on volume %s for file %s failed, %s\n",
                volume, file, strerror(errno));
        ret = -1;
        goto out;
    }
    ...
    if (prealloc) {
        ...
        if (ret) {
            nbd_err("glfs_zerofill() on volume %s for file %s failed, %s\n",
                    volume, file, strerror(errno));
            ret = -1;
            goto out;
        }
...
nbd_delete_file(int count, char **options)
{
    ...
    if (count != 3) {
        nbd_err("Invalid arguments for delete command!\n");
        return -1;
    }
    ...
    if (glfs_access(glfs, file, F_OK)) {
        nbd_err("file with name %s is not exist on volume %s\n",
                file, volume);
        ret = -1;
        goto out;
    }
    ...
    if (ret) {
        nbd_err("failed to delete file %s on volume %s\n",
                file, volume);
    }
        ...
        struct genl_info *info, void *arg)
{
    if (info->genlhdr->version != NBD_NL_VERSION) {
        nbd_err("cmd %d. Got header version %d. Supported %d.\n",
    ...
    case NBD_CMD_STATUS:
        ...
        break;
    default:
        nbd_err("Unknown netlink command %d!\n", cmd->c_id);
        return -EOPNOTSUPP;
    }
...
nbd_setup_netlink(void)
{
    ...
    if (ret < 0) {
        nbd_err("couldn't register family\n");
        goto err;
    }

    ret = genl_ops_resolve(sock, &nbd_ops);
    if (ret < 0) {
        nbd_err("couldn't resolve ops, is rbd.ko loaded?\n");
        goto err;
    }

    ret = genl_ctrl_resolve_grp(sock, "nbd", "nbd_mc_group");
    if (ret < 0) {
        nbd_err("couldn't resolve group, is nbd.ko loaded?\n");
        goto err;
    }

    ret = nl_socket_add_membership(sock, ret);
    if (ret < 0) {
        nbd_err("couldn't add membership\n");
        goto err;
    }
        ...
        int index)
{
    ...
    nbd_dbg("The file %s size: %llu, block size: %u\n", file, size, blk_size);
...
static int
nbd_socket_read(int fd, void *buf, size_t count)
{
    size_t cnt = 0;
...
static int
nbd_socket_write(int fd, void *buf, size_t count)
{
    while (count > 0) {
...
nbd_server_init(char *volume, char *host, char *file, int threads, int sock_fd)
{
    ...
    if (!gfd) {
        nbd_err("Failed to open the file\n");
        ret = -1;
        goto out;
    }
